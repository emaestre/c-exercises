...
template<class T> class Cola
{
	...
	public:
		Cola(void);
		inline Cola(const Cola<T>&);
		inline bool es_vacia(void) const; 
		inline T frente(void) const;
		void encolar(T);
...
template<class T> bool Cola<T>::es_vacia(void) const
{
	return prim == NULL;
}

template<class T> T Cola<T>::frente(void) const
{
	if(prim != NULL)
		return prim->consultarinfo();
}

template<class T> void Cola<T>::encolar(T e)
{
	Nodo<T> *aux,*aux2;
...
template<class T> Cola<T>& Cola<T>::operator= (const Cola<T> &c)
{
	copiar(c);
	return *this;
}
...
template<class T> void Cola<T>::copiar(const Cola<T> &c)
{
...
template<class T> Nodo<T>* Cola<T>::obtprim(void) const
{
	return prim;
}
