...
//Librerias usadas en la clase lista
#include "nodo.hpp"
#include "bloque.hpp"
#include<iostream>
#include<cstdlib>
...
template<class T> class Lista
{
	...
	public:	
		...
		//Metodos consultores
		inline bool es_vacia(void) const;
		inline int obtlongitud(void) const;
	...
	private:
		//Metodo para copiar una lista en otra
		void copiar(const Lista<T>&);
		//Obtener la direccion de memoria de el primer nodo de una lista
		inline Nodo<T>* obtprim(void) const;
		inline void modificarprim(Nodo<T>*);
		inline void modificarlongitud(int);
};
...
//Desarrollo del constructor copia
...
//Desarrollo de el metodo consultor es_vacia
template<class T> bool Lista<T>::es_vacia(void) const
{
	//Retornar la pregunta para saber si la lista es vacia
	return prim == NULL;
}

//Desarrollo de el metodo consultor obtlongitud
template<class T> int Lista<T>::obtlongitud(void) const
{
	//Retorno de la longitud
	return longitud;
}

template<class T> int Lista<T>::acumular(int pos) const
{
	Nodo<T> *aux;
	int acum,i;
...
template<class T> int Lista<T>::localizar(T e) const
{	
...
template<class T> T Lista<T>::consultar(int pos) const
{
	Nodo<T> *aux;
	T dato;	
	int i;
...
template<class T>  Lista<T> Lista<T>::unir(Lista<T> l)
{
	Nodo<T> *aux;
	Lista<T> l1;
...
template<class T>  Lista<T> Lista<T>::ordenar(void)
{
	Lista<T> l1,l2,l3;
	Nodo<T> *aux;	
...
template<class T>  void Lista<T>::insertar(int pos, T e)
{
	Nodo<T> *aux2;
...
template<class T> void Lista<T>::eliminar(int pos)
{
	Nodo<T> *aux,*aux2;
	int i;
...
template<class T> void Lista<T>::modificar(T e,int pos)
{
	Nodo<T> *aux;
	int i;
...
template<class T> void Lista<T>::separar(Lista<T> &l1, Lista<T> &l2)
{
	Nodo<T> *aux,*a,*b,*c;
	int i,j;
...
template<class T> void Lista<T>::separar(Lista<T> &l1, int pos)
{
	Nodo<T> *aux,*a,*c;
	int i,j;
...
template<class T> void Lista<T>::OcurrMayor(int &mayor, int &cont)
{
	Nodo<T> *aux2;	
	int aux;
...
template<class T> void Lista<T>::mezclar(Lista<T> &l1, Lista<T> &l2)
{
	Nodo<T> *aux3,*a,*b,*c;
	T aux, aux2;
...
template<class T> void Lista<T>::vaciar(void)
{
	Nodo<T> *aux;
...
template<class T> Lista<T> Lista<T>::compactar(Lista<T> &l1, Lista<T> &l2)
{
...
template<class T> void Lista<T>::descompactar(Lista<int> &l1, Lista<int> &l2)
{
	Nodo<T> *aux, *aux2, *a, *b; 
	int k,i,j;
	bool band;
...
template<class T> void Lista<T>::LIBK(Lista<Bloque> &LIB, int k)
{
	Nodo<Bloque> *aux, *aux3, *aux4;
	Bloque aux2;
	bool band;
	...
	while(aux != NULL)
	{
		if(aux->consultarinfo().consultarval() >= k)
		{
			aux3 = new Nodo<Bloque>;
			aux2.modificarval((aux->consultarinfo().consultarval()/k)*k);
			aux2.modificardir((aux->consultarinfo().consultardir())+(aux->consultarinfo().consultarval()%k));
			aux3->modificarinfo(aux2);
			aux3->modificarprox(prim);
			prim = aux3;
			aux2.modificarval(aux->consultarinfo().consultarval()%k);
			aux2.modificardir(aux->consultarinfo().consultardir());			
			aux->modificarinfo(aux2);
			
		}

		if(band)
			aux4 = aux4->consultarprox();			
					
		if(aux != LIB.obtprim() and !band)
		{				
			aux4 = LIB.obtprim();	
			band = true;
		}

		if(aux->consultarinfo().consultarval() == 0 and aux == LIB.obtprim())
		{
			LIB.modificarprim(LIB.obtprim()->consultarprox());
			delete aux;
			aux = LIB.obtprim();
		}else
		{
			if(aux->consultarinfo().consultarval() == 0 and aux != LIB.obtprim())
			{
				aux4->modificarprox(aux->consultarprox());
				delete aux;
				aux = aux4->consultarprox();
			}
			else
				aux = aux->consultarprox();
		}
...
template<class T> void Lista<T>::invertir(void)
{
	Nodo<T> *aux,*aux2;
...
template<class T> Lista<T>& Lista<T>::operator= (const Lista<T> &l)
{
	copiar(l);
	return *this;
}

template<class T> bool Lista<T>::operator== (const Lista<T> l) const
{
	Nodo<T> *aux,*aux2;
	bool band;
...
template<class T> void Lista<T>::copiar(const Lista<T> &l)
{
...
template<class T> Nodo<T>* Lista<T>::obtprim(void) const
{
	return prim;
}

template<class T> void Lista<T>::modificarprim(Nodo<T> *n)
{
	prim = n;
}

template<class T> void Lista<T>::modificarlongitud(int n)
{
	longitud = n;
}
#endif
